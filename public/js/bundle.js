...
var HistoryRecord = React.createClass({
    render: function(){
        var date=this.props.date.getDate()+'/'+this.props.date.getMonth()+'/'+this.props.date.getFullYear()+'   '+this.props.date.getHours()+':'+this.props.date.getMinutes()
        return (
            <tr>
                <td>
                    {date}
...
var History = React.createClass({
    ...
    render: function(){
        ...
        var content = this.props.feeds.map(function(feed){
            return (
                <HistoryRecord key={feed._id} date={new Date(feed.date)} amount={feed.amount} source={feed.source} />
            )
        });
...
var MilkLogger = React.createClass({
    ...
    handleFeedSubmit: function(feed) {
        ...
        feed.id = Date.now();
        var newFeeds = currentFeeds.concat([feed]);
        this.setState({feed: newFeeds});
        $.ajax({
            url: this.props.url,
            dataType: 'json',
            type: 'POST',
